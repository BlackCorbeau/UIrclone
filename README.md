...
- **Управление удалёнными хранилищами (remotes)** – просмотр, создание, проверка, удаление и просмотр информации обо всех подключениях rclone.
- **Создание хранилищ через GUI** – мастер добавления с пошаговыми вопросами rclone (`--non-interactive`): укажите имя и тип, а для облаков с авторизацией (Google Drive, Dropbox и др.) автоматически откроется браузер для входа в аккаунт (OAuth).
- **Контекстное меню хранилищ** – по правой кнопке мыши: открыть, проверить доступность, посмотреть использование (объём/число файлов), удалить с подтверждением.
...
2. На левой боковой панели отобразится список ваших rclone remote. Чтобы добавить новое хранилище, нажмите кнопку **«➕ Добавить облако»** и заполните мастер:
   - **Имя** – например, `mydrive`.
   - **Тип** – например, `drive`, `dropbox`, `s3`, `webdav`.
   - Если выбранный тип требует авторизации (OAuth), после нажатия «Создать» автоматически откроется браузер для входа в аккаунт. При необходимости rclone задаст дополнительные вопросы (например, о Team Drive) – ответьте в окне мастера.
3. Нажмите на имя remote **левой кнопкой мыши**, чтобы просмотреть его содержимое.
4. **Правая кнопка мыши** по хранилищу открывает контекстное меню:
   - **📂 Открыть** – перейти в хранилище.
   - **🔍 Проверить доступность** – проверить подключение.
   - **📊 Использование** – показать общий объём, занятое и свободное место, число файлов.
   - **🗑️ Удалить** – удалить хранилище из конфигурации rclone (файлы в облаке при этом не удаляются; требуется подтверждение).
5. Выберите файлы/папки, щёлкая по их иконке (📁 или 📄). Можно выбирать несколько элементов.
6. После выбора внизу появятся кнопки: **Копировать**, **Переместить**, **Удалить**.
7. Для копирования/перемещения укажите путь назначения:
...
8. Нажмите **Начать** – операция запустится в фоне. Прогресс отображается на правой панели.
...
- **Правая кнопка мыши по хранилищу** – контекстное меню хранилища.
...
├── operations/
│   ├── files.rs                # Обёртка над rclone lsjson / about
│   ├── sync.rs                 # copy/move/delete/purge
│   ├── remotes.rs              # list/get/check, интерактивное создание (config create --non-interactive)
...
- [ ] Графическое изменение конфигурации существующих remote.
